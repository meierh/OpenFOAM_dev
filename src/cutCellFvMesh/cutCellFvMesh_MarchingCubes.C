...
*/
/*
unsigned int Foam::cutCellFvMesh::MC33::surfint(unsigned int x, unsigned int y, unsigned int z, double *r) {
	r[0] = x; r[1] = y; r[2] = z;
	if (x == 0)
...
void Foam::cutCellFvMesh::MC33::find_case(unsigned int x, unsigned int y, unsigned int z, unsigned int i) {
...
/*
void Foam::cutCellFvMesh::MC33::case_count(unsigned int x, unsigned int y, unsigned int z, unsigned int i) {
	unsigned int p[13] = {FF,FF,FF,FF,FF,FF,FF,FF,FF,FF,FF,FF,FF};
	union {
		int f[6];
		unsigned int ti[3];
...
/*
void Foam::cutCellFvMesh::MC33::free_temp_D_U() {
...
/*
void Foam::cutCellFvMesh::MC33::clear_temp_isosurface() {
	if (F) {
...
#define code_in_define_part02\
			double t = invSqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);\
			float *q = S->N[nv].v;\
			*q = t * *r; *(++q) = t * *(++r); *(++q) = t * *(++r);\
			return nv;
...
/*
int Foam::cutCellFvMesh::MC33::set_grid3d(grid3d &G) {
...
/*
int Foam::cutCellFvMesh::M33::set_grid3d(grid3d *G) {
	if (!G)
		return -1;
	return set_grid3d(*G);
}
...
/*
int Foam::cutCellFvMesh::MC33::calculate_isosurface(surface &Sf, double iso) {
		...
		for (unsigned int y = 0; y != ny; ++y) {
			...
			for (unsigned int x = 0; x != Nx; ++x) {
				...
				if (i && i^0xFF) { //i is different from 0 and 0xFF
					if (v2 < v) {double *t = v2; double *s = t + 8; *s = *t; *(++s) = *(++t); *(++s) = *(++t); *(++s) = *(++t);}// memcpy(v2 + 8, v2, 4*sizeof(double));
					find_case(x, y, z, i);
				}
			}
		}
...
/*
size_t Foam::cutCellFvMesh::M33::size_of_isosurface(double iso, unsigned int &nV, unsigned int &nT) {
	const GRD_data_type ***FG = F;
	if (!FG)//The set_grid3d function was not executed
		return -2;
	surface Sf;
	...
	for (unsigned int z = 0; z != nz; ++z) {
		...
		for (unsigned int y = 0; y != ny; ++y) {
			...
			for (unsigned int x = 0; x != Nx; ++x) {
				...
				if (signbf(v2[2])) i |= 2;
				if (i && i^0xFF) {
					if (v > v2) {double *t = v2; double *s = t + 8; *s = *t; *(++s) = *(++t); *(++s) = *(++t); *(++s) = *(++t);}
					case_count(x, y, z, i);
				}
			}
		}
	...
	return nV*(3*(sizeof(double) + sizeof(float)) + sizeof(int)) + nT*(3*sizeof(int)) + sizeof(surface);
}

size_t Foam::cutCellFvMesh::M33::size_of_isosurface(double iso) {
	unsigned int nV, nT;
